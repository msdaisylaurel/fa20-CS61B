...
public class BuggyIntDList extends IntDList {
    ...
    private DNode sortedMerge(DNode d1, DNode d2) {
        ...
        if (d1.val <= d2.val) {
            sortedMerge(d1.next, d2); // FIXME: Replace this line (if needed). HINT: Step Into(F7) using debugger and try to figure out what it does.
            d1.next.prev = d1;
            d1.prev = null;
            return d1;
        } else {
            sortedMerge(d1, d2.next);   // FIXME: Replace this line (if needed). HINT: Step Into(F7) using debugger and try to figure out what it does.
            d2.next.prev = d2;
            d2.prev = null;
            return d2;
        }
